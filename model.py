...
user_input = st.text_input("Enter Stock Ticker", "Infosys Data")

infy=pd.read_csv("Downloads\Infosys.csv")

st.subheader("Data From 2016 till 2022")
st.write(infy)

st.subheader("Features vs Time Chart")
st.area_chart(infy[["Open","High","Low","Close"]])


st.subheader("Closing Price vs Time Chart with 100MA")
ma100 = infy.Close.rolling(100).mean()
...
ma100 = infy.Close.rolling(100).mean()
ma200 = infy.Close.rolling(200).mean()
fig = plt.figure(figsize = (12,6))
plt.plot(infy.Close)
plt.plot(ma100)
plt.plot(ma200)
...
data_scaled = normalizer.fit_transform(np.array(infy.Close.values).reshape(-1,1))
...
#Loading Model :
model = load_model(r"C:\Users\gmpl\Downloads\keras_model (2).h5")
